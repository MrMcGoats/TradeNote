...
export async function useUploadTrades(param99, param0) {
    ...
    const loopOpenPositionsParse = async () => {
        return new Promise(async (resolve, reject) => {
            for (let index = 0; index < openPositionsParse.length; index++) {
                const element = openPositionsParse[index];
                const opIndex = openPositionsFile.findIndex(x => x.id === element.id);
                if (opIndex === -1 && element.exitTime !== 0) { // Position not in file and has exit time
