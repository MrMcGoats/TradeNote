  ...
  <main class="container" id="registerSignup">
    ...
    <div v-else>
      <form class="text-center col-md-4 offset-md-4 mt-5" v-on:submit.prevent="pageId == 'login' ? login() : register()">
        ...
        <button class="mt-3 w-100 btn btn-lg btn-primary" type="submit" :disabled="signingUp">
          {{ pageId == 'login' ? "Log in" : "Register" }}
          <span v-if="signingUp" class="ms-2 spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
        </button>
      </form>
      <div class="text-center mt-3" v-if="!registerOff">
        <a :href="pageId == 'login' ? '/register' : '/'">{{ pageId == 'login' ? "Register" : "Login" }}</a> page
      </div>
    </div>
  </main>
</template>
